...
def run_callback(*args):
    ...
    objs = [l[1] for l in loc if torch.is_tensor(l[1])]
    if len(objs) > 0:
        if thr is not None:
            '''core.is_closed = True
            thr.join(.1)
            if thr.is_alive():
                del thr
                thr = None'''
            core.geometry_update_data = core.get_showable(objs, rgb)
        else:
            thr = StoppableThread(target=show_all, args=(objs, rgb))
            thr.start()
